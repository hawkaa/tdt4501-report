...
In \oracle, join indexes are bitmap indexes on foreign key columns, and is a space efficient way of reducing the join data volume by applying restrictions to the tables before entering the join \cite{noauthor_undated-xi}. As seen in Figure \ref{fig:oracle-join-index}, a \term{Join Index} stores a bitmap for every distinct value in a foreign column. Using join indexes, \oracle~may improve performance by an order of magnitude \cite{noauthor_undated-hp}.
